...
def open_links_in_browser(browser_name, browser_path, urls, profile=None):
    ...
    print(f"\nOpening {len(urls)} URLs...")
    
    if sys.platform == "darwin":  # macOS
        base_cmd = ["open", "-a", browser_path]
        
        # Add profile arguments for supported browsers
        if profile and profile != "default":
            if any(browser in browser_name for browser in ["Google Chrome", "Microsoft Edge", "Brave Browser"]):
                base_cmd.extend(["--args", "--profile-directory=" + profile])
            elif "Firefox" in browser_name:
                base_cmd.extend(["--args", "-P", profile])
        
        # Open URLs with a small delay between each
        for url in urls:
            try:
                cmd = base_cmd + [url]
        ...
        base_cmd = [browser_path]
        
        # Add profile arguments for supported browsers
        if profile and profile != "default":
            if any(name in browser_path.lower() for name in ["chrome", "edge", "brave"]):
                base_cmd.append("--profile-directory=" + profile)
            elif "firefox" in browser_path.lower():
                base_cmd.extend(["-P", profile])
        
        # Open URLs with a small delay between each
        for url in urls:
            try:
                cmd = base_cmd + [url]
                subprocess.run(cmd, capture_output=True, stderr=subprocess.PIPE, check=True)
                print(f"✓ Opened: {url}")
                time.sleep(0.5)  # Add small delay between opens
            except subprocess.SubprocessError as e:
                print(f"✗ Error opening {url}: {str(e)}")
